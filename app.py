...
@app.callback(Output("animated-graph", "figure"),
   [
...
def update_graph(n_clicks,node_number, n_nests, connectivity, N, theta, alpha,
                 a0,b,gamma,T):
    ...
    fig = generate_animation(node_number, n_nests, connectivity, N, theta, alpha,
                 a0,b,gamma,T)   
    return fig

def generate_animation(node_number, n_nests, connectivity, N, theta, alpha,
                 a0,b,gamma,T):
    # ... (your code for generating the figure using the provided values)
    G, neighbor, nests, node_num = graph_generator(N_node=node_number, 
                                                   n_nests=n_nests,
    ...
    fig.update_layout(xaxis1= axis,
                  ...
                  hovermode = "closest",
                  title = "{} Ants, {} Tree Nodes, Nests: {}, connectivity: {}".format(N, node_num, nests,connectivity)
                  )
