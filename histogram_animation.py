...
fig.add_trace(go.Bar(x=eta_df['node'], y=eta_df['pop'], name='Population Density', 
              marker_color=eta_df['pop'], opacity=0.8),row=1, col=2)
...
for i in range(T):
    ...
    # frame["data"].append(go.Histogram(x=eta_seq[i],
    ...
    #             name="population",
    #             opacity=0.8,
    #             marker={'color':('red','orange','yellow','green','blue','cyan','purple')}))
    eta_df_temp = pd.DataFrame(eta_seq[i])
    eta_df_temp['node']=eta_df_temp.index
    eta_df_temp = eta_df_temp.rename({0:'pop'},axis=1)
    
    frame["data"].append(go.Bar(x=eta_df_temp['node'], y=eta_df_temp['pop'], name='Population Density', 
              marker_color=eta_df['pop'], opacity=0.8))
